...
/**
 * Voltara-styled dropdown for expense entities. Mirrors BillCategoryPicker:
 * inline add + per-row delete (blocked by in-use validation against existing
 * expenses).
 */
export function ExpenseEntityPicker({ value, onChange }: Props) {
  const { data: entities = [] } = useExpenseEntities();
  const { data: expenses = [] } = useExpenses();
  const createMut = useCreateExpenseEntity();
  const deleteMut = useDeleteExpenseEntity();
  ...
  const handleAdd = () => {
    ...
    if (entities.includes(name)) {
      setError('Entity already exists.');
      return;
    }
  ...
  const handleDelete = (name: string) => {
    if (expenses.some((e) => e.entity === name)) {
      setError(`"${name}" is in use by existing expenses and can't be removed.`);
      setPendingDelete(null);
      return;
    }
      ...
      {open && (
        ...
        >
          <div style={{ maxHeight: 240, overflowY: 'auto' }}>
            ...
            {entities.map((ent) => {
              const isSelected = ent === value;
              const isPending = pendingDelete === ent;
              return (
                <div
                  key={ent}
                ...
                >
                  <span
                    style={{ flex: 1 }}
                    onClick={() => { onChange(ent); setOpen(false); }}
                  >
                    {ent}
                  </span>
                  {isPending ? (
                    ...
                    <button
                      type="button"
                      onClick={(e) => { e.stopPropagation(); setError(null); setPendingDelete(ent); }}
                      title="Remove entity"
                      style={{
