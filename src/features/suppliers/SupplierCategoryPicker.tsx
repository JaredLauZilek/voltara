...
export function SupplierCategoryPicker({ kind, value, onChange }: Props) {
  const { data: categories = [] } = useSupplierCategories(kind);
  const { data: suppliers = [] } = useSuppliers();
  const createMut = useCreateSupplierCategory(kind);
  const deleteMut = useDeleteSupplierCategory(kind);
  ...
  const handleDelete = (name: string) => {
    // Only block if a supplier of THIS kind still uses the category — categories are now scoped per kind.
    if (suppliers.some((s) => (s.kind ?? 'Supplier') === kind && s.category === name)) {
      setError(`"${name}" is in use by ${kind.toLowerCase()}s and can't be removed.`);
      setPendingDelete(null);
      return;
    }
